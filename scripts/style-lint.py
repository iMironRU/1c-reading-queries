...
def check_file(path, cfg):
    ...
    if sev("impersonal") != "off":
        ...
        # Обращение бывает и без местоимения — повелительным наклонением.
        # Перечень частый, а не полный: линтер ловит дёшево, остальное — редактор.
        imper = ("посмотрите|присмотритесь|обратите|попробуйте|представьте|заметьте|"
                 "сравните|откройте|возьмите|проверьте|вспомните|прочтите|найдите|"
                 "поищите|подумайте|учтите|смотрите")
        pat = re.compile(r"(?<![А-Яа-яЁё])(вы|вас|вам|вами|ваш[а-яё]*|"
                         + imper + r")(?![А-Яа-яЁё])", re.I)
    ...
    if sev("kitchen") != "off" and "/docs/" not in path.replace("\\", "/"):
        KITCHEN = [
            ...
            (r"^(?![ \t]*#).*?§\s*\d+\.\d+",
             ...
             None, "warn"),
            # Книга для переходящего с другого языка: опыта в целевой платформе
            # у читателя предполагать нельзя. Ловим действия, а не знание вообще:
            # «вы знаете» и «вы привыкли» чаще про его же родной язык.
            (r"(?<![А-Яа-яЁё])вы (?:пользуетесь|писали на|уже писали|делали это)\b[^.!?]{0,60}",
             "предполагает опыт читателя в изучаемой платформе — а он тут новичок",
            ...
            (r"(?<![А-Яа-яЁё])(фаза [ABCАБВ]|модул[ья] \d|концепт(?!уал)|предметк|"
             r"перв(?:ый|ого) заход|втор(?:ой|ого) заход|заход(?:ах|ами)|"
             r"реврейм|минимальн\w+ доз|"
    ...
    if sev("wide") != "off":
        for m in re.finditer(r"^```([^\n]*)\n(.*?)^```", raw, flags=re.S | re.M):
            ...
            # У кода порог свой. Схема или таблица из пробелов на переносе
            # рассыпается — там 46 знаков и есть предел. Код в EPUB переносится
            # с висячим отступом: продолжение сдвинуто вправо, и видно, что это
            # хвост предыдущей строки. Ломать осмысленную строку кода ради
            # узкого экрана хуже, чем дать ей перенестись. Порог 72 — та ширина,
            # за которой строку стоит укоротить и в обычном редакторе.
            # Код — это и `bsl`, и снипеты языков сравнения: они тут
            # ровно затем, чтобы читатель увидел знакомую строку целиком.
            is_code = lang.split(",")[0] in CODE_LANGS
            limit = 72 if is_code else 46
            if widest <= limit:
                continue
            ...
            if two_col:
                ...
                findings.append((sev("wide"), path, line, 1, "wide",
                                 f"строка кода в {widest} знаков — длинновата даже для "
                                 f"редактора, на телефоне переносится дважды"))
            else:
                findings.append((sev("wide"), path, line, 1, "wide",
